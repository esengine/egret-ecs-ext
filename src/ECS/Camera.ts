    ...
    export class Camera extends Component {
        ...
        public get bounds() {
            ...
            if (this._areBoundsDirty) {
                ...
                let bottomRight = this.screenToWorldPoint(new Vector2(Core.Instance.width - this._inset.right,
                    Core.Instance.height - this._inset.bottom));

                if (this.entity.transform.rotation != 0) {
                    // 特别注意旋转的边界。我们需要找到绝对的最小/最大值并从中创建边界
                    let topRight = this.screenToWorldPoint(new Vector2(Core.Instance.width - this._inset.right,
                        this._inset.top));
        ...
        protected updateMatrixes() {
            ...
            if (this.entity.transform.rotation != 0) {
                tempMat = Matrix2D.createRotation(this.entity.transform.rotation);
                this._transformMatrix = this._transformMatrix.multiply(tempMat);
            }
